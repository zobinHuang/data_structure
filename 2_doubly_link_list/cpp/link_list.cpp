...
int link_list::insert_node(data_type data, int insert_position)
{
    ...
    if(insert_position == 0){
        ...
        new_node->set_prior(temp);
        if(temp->get_next() != NULL)//important!
            temp->get_next()->set_prior(new_node);
...
int link_list::delete_node(int delete_position)
{
    ...
    if(delete_position == 0){
        ...
        if(old_node->get_next() != NULL)//important!
            old_node->set_prior(temp);
        delete(old_node);
    }
    length--;
    return 1;
